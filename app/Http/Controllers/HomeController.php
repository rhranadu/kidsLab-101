...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $student_courses_type1 = CourseBatch::with('courses','courseClasses')
            ->where([
                ['student_type','=',1],
                ['end_date','>=',$dt],
                ['status','>=',1]
            ])->get();
        $student_courses_type2 = CourseBatch::with('courses','courseClasses')
            ->where([
                ['student_type','=',2],
                ['end_date','>=',$dt],
                ['status','>=',1]
            ])->get();
        $student_courses_type3 = CourseBatch::with('courses','courseClasses')
            ->where([
                ['student_type','=',3],
                ['end_date','>=',$dt],
                ['status','>=',1]
            ])->get();
//        $abc = count($student_courses);
//        dd($student_courses_type3);

        return view('home', compact('category', 'sliders', 'facts', 'categories', 'cors', 'bundles', 'meetings', 'bigblue', 'testi', 'trusted', 'recent_course_id', 'blogs', 'subscriptionBundles', 'batches', 'recent_course', 'total_count', 'advs', 'allgooglemeet','jitsimeeting','student_courses_type1','student_courses_type2','student_courses_type3'));
    }
