...
class QuickUpdateController extends Controller
{
    ...
    public function sessionBatchQuick($id)
    {
        $sessionOrBatch = SessionBatch::findorfail($id);

        if($sessionOrBatch->status ==0)
        {

            DB::table('session_batches')->where('id','=',$id)->update(['status' => "1"]);
            return back()->with('success','Status changed to active !');
        }
        else
        {
            DB::table('session_batches')->where('id','=',$id)->update(['status' => "0"]);
            return back()->with('delete','Status changed to deactive !');
        }
    }
